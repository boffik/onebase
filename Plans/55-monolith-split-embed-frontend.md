...
- следующим срезом очищены query/dev и utility pages: `/ui/query-builder`,
  ...
  замыкании;
- следующим срезом очищены админки и ext-admin страницы: подтверждения удаления/
  завершения сессий/отзыва токенов переведены на `data-ob-confirm`, readonly
  API-токен выбирается через `data-ob-select-on-click`, а standalone admin
  templates подключают `/static/ui.js`;
- production UI больше не рендерит inline event attributes; следующий долг по
  этапу 3 — дальнейшее дробление крупных script-блоков в реальные static assets,
  если потребуется продолжить уменьшение шаблонных литералов.
