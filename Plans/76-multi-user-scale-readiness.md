...
последовательным и конкурентным regression-тестом. Остальные пункты этапа D про
межпроцессные `БлокировкаДанных`/PostgreSQL advisory locks остаются future work.
...
1. **D1:** атомарная optimistic locking запись. Это маленькая по объёму, но
   принципиальная корректность.
2. **B/C:** индексы + server-side reference picker. Это даст реальный выигрыш
   на больших справочниках.
3. **F:** slog/request_id/slow operations, чтобы следующие оптимизации делались
   по данным, а не на глаз.
4. **E:** лимиты и background export для тяжёлых отчётов.
