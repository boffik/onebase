...
Текущее состояние (разведка подтверждена): тип поля добавляется тривиально (`FieldType` — строка, голый `richtext` уже проходит парсер), TEXT-хранение и миграция готовы (дефолтная ветка `ddl.go fieldType`), CRUD строк без спецобработки. **Нет**: WYSIWYG-редактора, HTML-санитайзера (нет в go.mod), inline-вложений, вывода HTML в ячейку печатной формы (`sheet.Cell` экранирует `Text`).
