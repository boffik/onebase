...
- пользовательские DSL-пути (`Справочники`, `Документы`, `Запрос`,
  `РегистрыНакопления`, `ЗначениеРеквизитаОбъекта`) теперь проходят через
...
### 79F — storage-чокпоинт (defense-in-depth), по итогам ревью 2026-07-08

У query compiler есть финальная сеть `assertRowFiltersApplied`
(`internal/query/query.go`): если у источника с активной политикой предикат не
внедрён ни одним путём — запрос отклоняется (fail-closed). У списочных/объектных
путей (`store.List`/`GetByID`) такой сети **нет**: строковый фильтр применяется
по соглашению — каждый хендлер обязан позвать `applyRowFilter`/`rowAllowedID`
(см. `internal/ui/row_access.go`). Существующие хендлеры проверены и гейтят
корректно (напр. экспорт списка), но **новый** хендлер, дёрнувший
`s.store.List(...)` напрямую, тихо обойдёт RLS — регрессия не будет замечена.

Задача — убрать зависимость от дисциплины разработчика:

- вариант A (тест-сеть): тест-сканер проходит по list/get-хендлерам и падает,
  если путь к `store.List/GetByID` для ограничиваемого объекта не проходит через
  `applyRowFilter`/`rowAllowed*` (аналог того, как `assertRowFiltersApplied`
  ловит формы запросов);
- вариант B (рантайм-гейт): на storage-слое пометка «у объекта есть активная
  политика, но в `ListParams`/`GetByID` не передан RowFilter» → ошибка в
  dev/строгом режиме (fail-closed), чтобы пропуск фильтра всплывал сразу;
- предпочтителен A (нулевая рантайм-стоимость) + B под флагом строгого режима
