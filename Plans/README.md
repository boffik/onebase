...
> раздел «Аудит 2026-08-02» ниже). Итог: 58 закрыто, 49 частично, 7 не начато,
...
116 планов: **58 закрыто, 49 частично, 7 не начато, 2 отменены**.
...
| Д3 | Веб-хуки не срабатывают на DSL-пути записи | `ui/dsl_documents.go` не публикует события; их шлёт только `entityservice` | Документы из обработок, HTTP-сервисов и регламентов интеграции не видят |
| Д4 | Регистры с реквизитами исключены из итогов | `Register.TotalsUsable()` → только `len(Attributes)==0` | На таких регистрах ускорения ×150 из плана 80 нет |

### Текущий приоритет

1. **Дефекты Д1–Д4** — корректность учёта, дёшево и локально.
2. **План 109** — `errcheck` (~750 находок в prod: транзакции, бэкапы, запись
   файлов) и `gosec` (~945 не-G104). Этапы A (`bodyclose`, PR #503), B (`unused`,
   PR #508) и класс «тесты» (PR #511–#520, 0 находок в `_test.go`) закрыты.
   Осталось: этап C (`--new-from-merge-base` в CI), 109D–109G (prod по классам
   риска), 109H–109J (gosec). ≈9.5–15.5 дня.
...
| 88 | [88-field-level-masking-pii.md](88-field-level-masking-pii.md) | Маскирование полей ПДн по ролям + аудит раскрытия | 🟡 88A–88D закрыты (ассерты — план 113). **88E не сделан**: отчёт/виджет/AI-запрос с чувствительной колонкой под не-админом отклоняется вместо маскирования проекции, а DSL-прокси (`dsl_catalogs.go`, `dsl_documents.go`) отдаёт реальные значения мимо маскирования |
...
| 36 | [36-dsl-language-roadmap.md](36-dsl-language-roadmap.md) | 🟡 Приоритет 1 закрыт. Из приоритета 2 отсутствуют: `Выбор Когда` в скриптовом DSL, именованные параметры, деструктуризация, `Символы.ПС/Таб`, `XMLСтрока/XMLЗначение`, `Подождать`, `СгенерироватьGUID`, `ОбработкаПроверкиЗаполнения`, глобальные подписки на события |
