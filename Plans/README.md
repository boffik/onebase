...
> (проект в плане **79**).
> Сверка 2026-07-08: **79** (строковый доступ) и **77** (изоляция лексических
> областей DSL) влиты в main; query compiler внедряет row-предикаты fail-closed
> (`assertRowFiltersApplied`). Ревью зрелости добавил планы **80–86**
> (итоги регистров, реструктуризация схемы, полнотекстовый поиск, секреты,
...
> Сверка 2026-07-31 (аудит по коду перед фиксацией роадмапа): подтверждено
> реализованным то, что таблицы держали как ⬜/проект — **79** (строковый доступ,
> core влит), **80** (итоги регистров накопления: инкремент в той же транзакции,
> быстрый путь `query`, CLI `recalc-totals`), **85** (маршрут согласования:
> задачи/состояния — фаза 1), **87** (реалтайм-события форм), **88**
> (маскирование ПДн по ролям + аудит), **89** (версионируемые правила валидации),
> **90** (карантин/replay входящих + DLQ), **91** (протокол проверки результата в
> ai-guide), **99** (отладчик DSL — `internal/debugger/*`), **100** (действия
> ИИ-ассистента — `ui/ai_actions.go`). Планы **104–113** сведены в таблицу ниже
> (раздел «М»). План **76** (масштаб 100+) закрыт планом **111**. Остаются
> открытыми: из **80** — итоги бухрегистра `акк_*` + бенч/свёртка; из **79** —
> только этап **79F**; полностью не начаты **81/82/83/84**; **109** — этап A.

## Текущий приоритет, 2026-07-31

После закрытия арок масштабируемости (**111**), тест-инструментария (**108**) и
...
2. **План 80, продолжение** — итоги **бухрегистра** (`акк_*`). Механизм для
   регистров накопления готов и в `main` (инкремент в транзакции, быстрый путь
   `query`, CLI `recalc-totals`); осталось перенести его на план счетов + бенч
   `BenchmarkBalances` и переиспользование `RecalcTotals` в свёртке (план 74).
...
4. Стратегические направления, каждое — отдельный проект: **81** (безопасная
   ...
   фаза 1 в `main`), **86** (обмен данными — фаза 1 в `main`).

Осознанно отложено (из плана 111): **P3-1** горизонтальное масштабирование
...
Хвосты вне ядра: **60B** (marketplace конфигураций — часть A готова), **55** этап 3
(остаток template-bound inline-JS в `ui/templates.go`), **46** (store wrapper) и
остатки i18n/PWA — скорее приёмка и маркетинг.
...
| № | Файл | Фича | Эстимейт | Статус |
|---|---|---|---|---|
| 80 | [80-register-totals.md](80-register-totals.md) | Итоги регистров (предрасчёт остатков вместо `SUM` на лету) — ключ к масштабу учёта | 6–9 дней | 🟢 Регистры накопления реализованы: итоги в той же транзакции, быстрый путь `query` (Остатки/Остатки(&Момент)/ОстаткиИОбороты), CLI `recalc-totals`. Осталось: бухрегистр `акк_*`, бенч, свёртка |
| 81 | [81-schema-restructuring-safety.md](81-schema-restructuring-safety.md) | Безопасная реструктуризация схемы: устойчивые id полей, rename/retype/drop с данными, dry-run | 7–10 дней | ⬜ Проект |
| 82 | [82-fulltext-search.md](82-fulltext-search.md) | Полнотекстовый / глобальный поиск (PG tsvector, SQLite FTS5) с учётом RBAC+RLS | 6–8 дней | ⬜ Проект |
...
| № | Файл | Фича | Статус |
|---|---|---|---|
| 87 | [87-form-realtime-events.md](87-form-realtime-events.md) | Реалтайм-события форм (SSE, обновление без перезагрузки) | ✅ Реализовано |
| 88 | [88-field-level-masking-pii.md](88-field-level-masking-pii.md) | Маскирование полей ПДн по ролям + аудит раскрытия | ✅ Реализовано (ассерты — план 113) |
| 89 | [89-versioned-validation-profiles.md](89-versioned-validation-profiles.md) | Версионируемые профили правил валидации | ✅ Реализовано |
| 90 | [90-inbound-intake-idempotency-dlq.md](90-inbound-intake-idempotency-dlq.md) | Идемпотентность входящих, карантин/DLQ, replay | ✅ Реализовано |
...
| 99 | [99-debugger.md](99-debugger.md) | Отладчик DSL (точки останова, шаги, стек) — `internal/debugger/*` | ✅ Реализовано |
