...
func TestAnalyze_TextsTemplate(t *testing.T) {
	// Analyze резолвит шаблон относительно CWD («templates/texts») — из
	// каталога пакета он не виден, поэтому переходим в корень репозитория.
	if !dirExists("../../templates/texts") {
		t.Skip("templates/texts/ not found (run from repo root)")
	}
	t.Chdir("../..")
