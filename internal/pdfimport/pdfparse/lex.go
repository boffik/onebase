...
func (b *buffer) reload() (bool, error) {
	...
	if n == 0 && err != nil {
		...
		_ = b.errorf("malformed PDF: reading at offset %d: %v", b.offset, err)
		return false, err
	}
	b.offset += int64(n)
	b.buf = b.buf[:n]
	...
	for {
		...
		case '\\':
			...
			default:
				...
				_ = b.errorf("invalid escape sequence \\%c", c)
				tmp = append(tmp, '\\', c)
			case 'n':
				tmp = append(tmp, '\n')
			case 'r':
...
func (b *buffer) readKeyword() token {
	...
	case isInteger(s):
		...
		if err != nil {
			...
			_ = b.errorf("invalid integer %s", s)
		}
		return x
	case isReal(s):
			...
			case keyword("obj"):
				...
				if _, ok := obj.(stream); !ok {
					...
					if tok4 != keyword("endobj") {
						...
						_ = b.errorf("missing endobj after indirect object definition")
						b.unreadToken(tok4)
					}
				}
				b.objptr = old
...
func (b *buffer) readDict() object {
	...
	for {
		...
		if !ok {
			...
			_ = b.errorf("unexpected non-name key %T(%v) parsing dictionary", tok, tok)
			continue
		}
		res, err := b.readObject()
		if err != nil {
