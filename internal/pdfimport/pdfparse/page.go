						...
						if len(bfrange.lo) == n && bfrange.lo <= text && text <= bfrange.hi {
							if bfrange.dst.Kind() == String {
								...
								if bfrange.lo != text { // value isn't at the beginning of the range so scale result
									// OneBase-патч (план 64): прибавляем полное
									// числовое смещение (text-lo) ко ВСЕМ байтам
									// назначения как к big-endian целому, с
									// переносом. Исходный код инкрементировал
									// только последний байт, из-за чего широкие
									// identity-диапазоны fpdf «<0000> <FFFF>
									// <0000>» теряли старший байт (U+041D → U+001D).
									s = addRangeOffsetBE(s, text, bfrange.lo)
								}
								r = append(r, []rune(utf16Decode(s))...)
								continue Parse
							}
							if bfrange.dst.Kind() == Array {
								n := offsetBE(text, bfrange.lo)
								v := bfrange.dst.Index(n)
								if v.Kind() == String {
									s := v.RawString()
									r = append(r, []rune(utf16Decode(s))...)
									continue Parse
...
// offsetBE возвращает числовую разность (code - lo), трактуя обе строки как
// big-endian целые равной длины. Используется для индексации bfrange-массивов и
// масштабирования строкового назначения (OneBase-патч, план 64).
func offsetBE(code, lo string) int {
	off := 0
	for i := 0; i < len(code) && i < len(lo); i++ {
		off = off<<8 + int(code[i]) - int(lo[i])
	}
	return off
}

// addRangeOffsetBE прибавляет смещение (code-lo) к строке назначения dst,
// трактуя dst как big-endian целое с переносом по байтам. Исправляет потерю
// старшего байта в широких identity-bfrange (fpdf «<0000> <FFFF> <0000>»):
// исходный rsc.io/pdf увеличивал только последний байт назначения.
func addRangeOffsetBE(dst, code, lo string) string {
	delta := offsetBE(code, lo)
	b := []byte(dst)
	carry := 0
	for i := len(b) - 1; i >= 0; i-- {
...
func readCmap(toUnicode Value) *cmap {
	...
	Interpret(toUnicode, func(stk *Stack, op string) {
		...
		case "defineresource":
			...
			stk.Push(value)
		default:
			// неизвестный оператор CMap — игнорируем
		}
	})
	if !ok {
		return nil
