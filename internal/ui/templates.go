...
<main class="main-list">
  ...
  {{range .PageBlocks}}
    {{if eq .Kind "heading"}}<h3>{{.Text}}</h3>
    {{else if eq .Kind "paragraph"}}<p style="margin-bottom:14px;color:#334155;font-size:14px;line-height:1.55;max-width:900px">{{.Text}}</p>
    {{else if eq .Kind "kpi"}}<div class="card" style="display:inline-block;min-width:200px;margin:0 12px 14px 0;padding:16px 20px;vertical-align:top">{{if .Label}}<div style="font-size:12px;text-transform:uppercase;letter-spacing:.05em;color:#64748b;font-weight:600;margin-bottom:6px">{{.Label}}</div>{{end}}<div style="font-size:28px;font-weight:700;color:#0f172a;line-height:1.1">{{.Value}}</div></div>
    {{else if eq .Kind "button"}}<a href="{{.URL}}" class="btn btn-primary" style="margin:0 8px 14px 0">{{.Text}}</a>
    {{else if eq .Kind "divider"}}<hr style="border:none;border-top:1px solid #e2e8f0;margin:18px 0;max-width:1400px">
    {{else if eq .Kind "raw"}}<div class="card" style="margin-bottom:14px">{{pageRaw .HTML}}</div>
    ...
    <div class="card" style="margin-bottom:14px;overflow-x:auto">
      ...
      <table>
        ...
        <tbody>
        {{range .Rows}}{{$row := .}}<tr>{{range $cols}}{{$c := index $row.Cells .}}<td>{{if $c.URL}}<a href="{{$c.URL}}" style="color:#3b82f6;text-decoration:none">{{$c.Text}}</a>{{else}}{{$c.Text}}{{end}}</td>{{end}}</tr>
        {{end}}
        </tbody>
      </table>
    </div>
