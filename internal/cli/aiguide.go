...
- Команды рисуются ТОЛЬКО элементом kind: Кнопка (обработчик <Имя>Нажатие в .form.os).
  Секция commands: сама кнопок НЕ создаёт — объявленная там команда без размещённой
  кнопки в UI не видна (валидатор: W014).
- Реквизит формы (attributes с save: false) ссылочного/перечислимого типа НЕ даёт
  пикер выбора: пикер/список получают только поля сущности (data_path: Объект.X). Для
  выбора из справочника используй поле объекта, не реквизит формы (валидатор: W015).
- Булево-по-смыслу поле объявляй type: bool (рендерится «Да/Нет»), не type: number —
  иначе оператор вводит 0/1 вручную.
