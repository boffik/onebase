...
Настройка самого обратного прокси (HTTP/2 наружу, тюнинг SSE `/ui/events`,
опциональный `ONEBASE_H2C=1` для HTTP/2 к апстриму) — в
[docs/reverse-proxy.md](reverse-proxy.md). Для 100+ активных пользователей это
обязательный шаг: без HTTP/2 на стороне браузера live-обновления (SSE) упираются
в лимит браузера «6 соединений на origin».
