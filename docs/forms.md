...
с системным модификатором браузера/ОС (например, Alt/Option + клавиша). Для
настоящих клавиш F2/F7/F8 нужен отдельный механизм `hotkey`, это не поведение
HTML `accesskey`. Платформенная `command_bar` и F-key hotkeys остаются отдельной
runtime-задачей.
