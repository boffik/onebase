...
- при необходимости поднять размер пула (по умолчанию приложение ставит
  `pool_max_conns=20`); задаётся в `app.yaml` секцией `db:` или параметром DSN;
...
- PostgreSQL: пул pgxpool настраивается приложением с дефолтами
  `pool_max_conns=20`, `pool_min_conns=2` (план 111, P0-1) вместо прежнего
  pgx-дефолта `max(4, NumCPU)`. Переопределяется в `app.yaml` секцией `db:`
  (`pool_max_conns`/`pool_min_conns`) или параметрами DSN; app.yaml имеет
  приоритет над DSN. При `--config-source=database` app.yaml лежит в БД и на
  первичное подключение не влияет — там размер пула задаётся DSN. Эффективный
  `max_conns` логируется при старте; насыщение мониторить по `onebase_db_pool_*`.
